...
export class RequestValidator {
    ...
    ValidateString(text: string, options?: { min: number, max: number, regex?: RegExp, err_msg?: string }) {
        if (options) {
            const { min, max, regex, err_msg } = options
            if (regex && !regex.test(text)) throw ErrMsg(ERROR_CODE.INVALID_PARAMS, err_msg || `string invalid with regex=${regex}`)
            if ((min && text.length < min) || (max && text.length > max)) throw ErrMsg(ERROR_CODE.INVALID_PARAMS, err_msg || `string must be in [${min || 0},${max || Infinity}]`)
        }
    }
}
