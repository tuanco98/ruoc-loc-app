import { MILLISECOND_PER_ONE_DAY, MILLISECOND_PER_ONE_SEC, SECOND_OF_ONE_DAY, SECOND_OF_ONE_HOUR } from "./constants"
...
class TimeSystem {
    ...
    getDateInFuture(current: Date, options?: {
        seconds?: number
        minutes?: number
        hours?: number
        days?: number
        ...
        return new Date(current.getTime() + next_seconds * MILLISECOND_PER_ONE_SEC + next_minutes * MILLISECOND_PER_ONE_SEC * 60 + next_hours * MILLISECOND_PER_ONE_SEC * SECOND_OF_ONE_HOUR + next_days * MILLISECOND_PER_ONE_DAY)
    }
}

export const timeSystem = new TimeSystem({ msOfOneDay: SECOND_OF_ONE_DAY * 1000 })